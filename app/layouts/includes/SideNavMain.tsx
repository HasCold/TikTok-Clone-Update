...
const SideNavMain = () => {
    ...
    let {setRandomUsers, randomUsers, user, token} = useGeneralStore();
    let {currentProfile} = useProfileStore();
    const pathname = usePathname();
    const contextUser = useUser();

    useEffect(() => {setRandomUsers(token)}, []);
    ...
    return (
        ...
        id='SideNavMain'
        className={`
        max-[600px]:hidden fixed z-20 bg-white pt-[70px] h-full lg:border-r-0 border-r w-[75px] overflow-auto
